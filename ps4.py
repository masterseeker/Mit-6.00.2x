...
def r_squared(y, estimated):
    ...
    numerator = 0
    denominator = 0
    sampleMean = sum(y) / len(y)
    for i in range(len(y)):
        numerator += (y[i] - estimated[i]) ** 2
        denominator += (y[i] - sampleMean) ** 2
...
def evaluate_models_on_training(x, y, models):
    ...
    for model in models:
        pylab.plot(xCoords, yCoords, "bo", label = "Sample's Data Points")
        pylab.title("")
        pylab.xlabel("Year")
        pylab.ylabel("Temperature Celcius")
         
        estValues = np.polyval(model, xCoords)
        rSquared = r_squared(yCoords, estValues)
        pylab.plot(xCoords, estValues, "r-", label = "R^2 = " + str(rSquared))
